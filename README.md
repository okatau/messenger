...
Cейчас реализовано:
  ...
  * **Система друзей.** Список друзей, входящие заявки, принятие/отклонение — всё интегрировано во фронтенд в отдельной вкладке.

## Что изменилось

### Инфраструктура
- Удалены отдельные `Dockerfile.local` / `Dockerfile.prod` во всех трёх сервисах — теперь используется единый `Dockerfile`.
- `docker-compose.local.yml` обновлён: сервисы получают конфиг через переменные окружения (`APP_ENV_FILE`, `APP_CONFIG_FILE`), добавлены зависимости между контейнерами (auth ждёт Redis, chat ждёт friends).
- **Makefile** расширен: добавлены команды для staging-окружения (`staging-up/down/logs`) и CI/CD пайплайна с Docker Registry (`build-prod`, `push-prod`, `release-prod`). Версия образа берётся из `git rev-parse --short HEAD`.

### Сервисы
- **Логирование:** удалены кастомные middleware-логгеры (`middleware/logger.go`) и пакеты `pkg/logger/logger.go` из всех трёх сервисов — логирование перешло на встроенный механизм Echo.
- **chat_service:** удалён хендлер `GET /:roomId/active`; маршрут получения участников комнаты переименован в `GET /:roomId/users`.
- **friends_service:** добавлены методы репозитория `IsFriend`, `GetInvites`, `GetFriendByUsername` — необходимы для полноценной работы системы друзей.

### Фронтенд
- В левой панели появились две вкладки: **Чаты** и **Друзья**.
- Вкладка «Друзья» отображает список друзей и входящие заявки с возможностью принять/отклонить.
- Добавлена возможность искать пользователей и отправлять им заявку в друзья прямо из интерфейса.

### Тесты
- Добавлены тесты для репозитория сообщений (`chat_service/internal/repository/message_test.go`).
- Обновлены тесты во всех трёх сервисах под новую структуру компонентов.
...
| `GET` | `/:roomId/users` | Активные пользователи в комнате |
